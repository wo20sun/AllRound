...
public class ViewHolderHelper extends RecyclerView.ViewHolder {
    ...
    //这个是为了解决 recyclerview嵌套显示不全的问题 第一个adapter使用这个渲染view，第二个adapter使用上面那个渲染view
    public static ViewHolderHelper get(Context context, View convertView, int layoutId, int position) {
        if (convertView == null) {
            View itemView = LayoutInflater.from(context).inflate(layoutId,null);
            ViewHolderHelper holder = new ViewHolderHelper(context, itemView, null, position);
            holder.mLayoutId = layoutId;
            return holder;
        } else {
            ViewHolderHelper holder = (ViewHolderHelper) convertView.getTag();
    ...
    public ViewHolderHelper setOnCheckedListener(int viewId,
